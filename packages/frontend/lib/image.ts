...
export class Image {
  ...
  private _hexColors!: string[];
  private _isStartTop: boolean;
  ...
  public toRLE(
    ...
    if (!this._rle) {
      const { encodedImage, hexColors } = this.encode(getRgbaAt, colors);
      this._rle = encodedImage;
      this._hexColors = hexColors;
    }
    return { rle: this._rle, hexColors: this._hexColors };
  }
  ...
  private encode(
    ...
    if (rowCount) {
      // FIXME: set 0 if it use most left column
      this._bounds.left = Math.min(
        ...Object.values(this._rows).map((r) => r.bounds.left)
      );
      this._bounds.right = Math.max(
        ...Object.values(this._rows).map((r) => r.bounds.right)
      );
      // Exit early if image is empty
      const [rect] = this._rows[0]?.rects || [];
      if (rowCount === 1 && this.isEmptyRow(rect)) {
        return { encodedImage: "0x0000000000", hexColors };
      }
    }
  ...
  private updateImageBounds(y: number): void {
    ...
    if (
      !this.isEmptyRow(rects[0]) &&
      this._bounds.top === 0 &&
      !this._isStartTop
    ) {
      this._isStartTop = true;
      this._bounds.top = y;
    }

    if (this._bounds.top !== 0 || this._isStartTop === true) {
    ...
    this._rows[y].bounds = {
      left: rects[0].length === this._width ? 0 : rects[0].length,
      right:
        rects[0].length === this._width
          ? this._width
          : this._width - rects[rects.length - 1].length,
    };
  }
