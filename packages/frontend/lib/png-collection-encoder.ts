...
export class PNGCollectionEncoder implements IEncoder {
   ...
   */
  public encodeImage(name: string, png: PngImage, folder?: string) {
    const image = new Image(png.width, png.height);
    const { rle, hexColors } = image.toRLE(
      (x, y) => png.rgbaAt(x, y),
      this._colors
    );
  ...
  private format(flatten = false) {
    ...
    if (!flatten && folders.length) {
      data = folders.reduce<Record<string, EncodedImage[]>>(
